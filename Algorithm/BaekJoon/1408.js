...
const timeToSec = (time) => time[0] * 3600 + time[1] * 60 + time[2];
const twoDigitFormat = (time, timeType = 1) => Math.floor(time / timeType) < 10 ? `0${Math.floor(time / timeType)}` : Math.floor(time / timeType);
const secToTime = (time) => {
  let arr = new Array(3);
  for (let i = 0; i < 3; i++) {
    const timeType = 60 ** (2 - i);
    arr[i] = twoDigitFormat(time, timeType);
    time %= timeType;
  }
  return arr;
}
const timeDiff = timeToSec(b) - timeToSec(a);
console.log(secToTime(timeDiff < 0 ? 86400 + timeDiff : timeDiff).join(':'));
