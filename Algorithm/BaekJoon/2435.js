 ...
 */
const [[N, K], T] = require('fs').readFileSync('./input.txt', 'utf-8').trim().split('\n').map(e => e.split(' ').map(Number));
let arr = new Array(N + 1).fill(0);
let max;
for (let i = 1; i <= N; i++) arr[i] = arr[i - 1] + T[i - 1];
for (let i = K; i <= N; i++) {
  const diff = arr[i] - arr[i - K];
  max = !max ? diff : max < diff ? diff : max;
}
console.log(max);