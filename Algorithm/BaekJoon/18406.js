 ...
 */
const N = require('fs').readFileSync('./input.txt', 'utf-8').trim().split('').map(Number);

// 자릿수 분리 (prev, next)
const prev = N.splice(0, N.length / 2).reduce((acc, cur) => acc + cur, 0);
const next = N.reduce((acc, cur) => acc + cur, 0);

// 출력 판정 (result)
const result = prev === next ? 'LUCKY' : 'READY';
console.log(result);