 ...
 */
const [[n, m], T] = require('fs').readFileSync(0, 'utf-8').trim().split('\n').map(e => e.split(' ').map(BigInt));
const prefixSum = new Array(n + 1n).fill(0n);
for (let i = 1n; i <= n; i++) prefixSum[i] = prefixSum[i - 1n] + T[i - 1n];
const prefixSumX = new Array(n + 1n - m).fill(0n);
for (let j = 0n; j <= n - m; j++) prefixSumX[j] = prefixSum[j + m] - prefixSum[j];
let result = 0n;
prefixSumX.forEach(e => result = result < e ? e : result);
console.log(`${result}`);
