 ...
 */
const [n, ...N] = require('fs').readFileSync('./input.txt', 'utf-8').trim().split('\n').map(Number);
const num = Math.round(n * 0.15);
console.log(n !== 0 ? Math.round(N.sort((a, b) => a - b).slice(num, n - num).reduce((acc, cur) => acc + cur, 0) / (n - num * 2)) : 0);