 ...
 */
const [n, s] = require('fs').readFileSync('./input.txt', 'utf-8').trim().split('\n').map(e => e.split(' ').map(Number).sort((a, b) => a - b));
const pair = new Array(n[0]);
for (let i = 0; i < n[0]; i++) pair[i] = s[i] + s[s.length - i - 1];
const result = Math.min(...pair);
console.log(result);