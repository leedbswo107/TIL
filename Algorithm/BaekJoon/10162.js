 ...
 * 각 버튼마다 일정한 시간이 지정되어 있어 해당 버튼을 한번 누를 때마다 그 시간이 동작시간에 
 * 더해진다. 버튼 A, B, C에 지정된 시간은 각각 5분, 1분, 10초이다.
 ...
 */
const T = +require('fs').readFileSync('./input.txt', 'utf-8').trim();
const solution = (t) => {
  let result = [0, 0, 0];
  let time = t;
  while (time >= 10) {
