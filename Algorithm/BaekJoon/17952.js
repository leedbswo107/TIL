 ...
 * 성애가 받을 과제 점수를 출력한다.
 */
let [_, ...assignments] = require('fs').readFileSync('input.txt', 'utf-8').trim().split('\n');
const proceed = [];
let result = 0;
assignments = assignments.map(e => e.split(' ').map(Number));
assignments.forEach(e => {
  if (e[0] === 1) {
    proceed.push([e[1], e[2] - 1]);
    if (e[2] - 1 === 0) {
      result += e[1];
      proceed.pop();
    }
    ...
    if (proceed.length !== 0) {
      if (proceed[proceed.length - 1][1] - 1 === 0) {
        result += proceed[proceed.length - 1][0];
        proceed.pop();
      } else proceed[proceed.length - 1][1] -= 1;
    }
  }
});
console.log(result);