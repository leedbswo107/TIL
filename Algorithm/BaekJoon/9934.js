...
const building = input.split(' ').map(Number);
const solution = (arr, k) => {
  if (arr.length === 1) return result[k].push(arr[0]);
  const [start, end] = [0, arr.length - 1];
  const mid = Math.floor((start + end) / 2);
  result[k].push(arr[mid]);
  solution(arr.slice(0, mid), k + 1);
  solution(arr.slice(mid + 1,), k + 1);
}
solution(building, 0);
