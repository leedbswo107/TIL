 ...
 * 상근이는 자신이 알고있는 참가자의 수가 정확하다고 생각한다. 각 신문 기사에 실려있는
 * 참가자의 수가 몇 명 만큼 잘못되어있는지 구하는 프로그램을 작성하시오.
 *
 * 출력
 * 출력은 첫째 줄에 다섯 개의 숫자를 출력해야 한다. 이 숫자는 상근이가 계산한 참가자의 수와
 * 각 기사에 적혀있는 참가자의 수의 차이이다.
 */
const [i1, i2] = require("fs")
  .readFileSync("./input.txt", "utf-8")
  .trim()
  .split("\n");
const [L, P] = i1.split(" ").map(Number);
const total = L * P;
let attendee = i2.split(" ").map(Number);
attendee = attendee.map((e) => e - total);
console.log(attendee.join(" "));
